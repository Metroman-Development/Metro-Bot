...
- **Special Emojis**: $rendimiento, $equipamiento, $tecnico, $tren, $fabricante, $años`
        ...
        let color = await this._askQuestion(message, 'Enter embed color (hex without #) or type "default" for default blue (3498DB):', false);
        color = color.toLowerCase() === 'default' ? '3498DB' : color;

        let thumbnail = await this._askQuestion(message, 'Enter thumbnail URL or $keyword (type "skip" to omit):\nExamples: $metroLogo, $logoV4, $logoColors', false);
        thumbnail = this._processKeywords(thumbnail);

        let image = await this._askQuestion(message, 'Enter main image URL or $keyword (type "skip" to omit):\nExamples: $metroLogo, $logoV4, $logoColors', false);
        image = this._processKeywords(image);

        let authorName = await this._askQuestion(message, 'Enter author name or type "skip" to use your username:', false);
        let authorIcon = await this._askQuestion(message, 'Enter author icon URL or $keyword (type "skip" to omit):\nExamples: $line1, $bip, $tne', false);
        authorIcon = this._processKeywords(authorIcon);
        let authorUrl = await this._askQuestion(message, 'Enter author URL or type "skip" to omit:', false);

        const fields = [];
        while (true) {
            const addField = await this._askQuestion(message, 'Add a field? (yes/no)', false, ['yes', 'no', 'y', 'n']);
            ...
            const inline = await this._askQuestion(message, 'Display inline? (yes/no)', false, ['yes', 'no', 'y', 'n']);
            fields.push({
                name: fieldName,
                value: fieldValue,
        ...
        let footerText = await this._askQuestion(message, 'Enter footer text or type "skip" for default (supports $keywords):', false);
...
module.exports = new NewsCommand();