...
class AmenitiesResultsManager extends BaseButton {
    constructor() {
        super({
            customIdPrefix: 'amenitiesResults',
    ...
    async handleInteraction(interaction, metadata) {
        ...
        try {
            ...
            console.error('Failed to update amenities results:', error);
    ...
    _createResultsMessage(cacheData) {
        ...
        const embed = new EmbedBuilder()
            .setTitle(`🏪 Estaciones con Elemento Cultural: ${query}`)
            .setColor('#00A0E0') // Different color to distinguish from commerce
            .setFooter({ 
        ...
        pageResults.forEach(station => {
            ...
            // Add amenity emojis to each matching item
            const matchedItems = station.matching.map(item => {
                const emoji = config.culture[item.toLowerCase()] || '▪️';
                return `${emoji} ${item}`;
            });
    ...
    _createPaginationButtons(cacheData) {
        ...
        row.addComponents(
            new ButtonBuilder()
                .setCustomId(`amenitiesResultsPrev:${this.customIdPrefix}:${query}:${userId}`)
                .setLabel('◀')
        ...
        row.addComponents(
            new ButtonBuilder()
                .setCustomId('amenitiesResultsPageCount')
        ...
        row.addComponents(
            new ButtonBuilder()
                .setCustomId(`amenitiesResultsNext:${this.customIdPrefix}:${query}:${userId}`)
                .setLabel('▶')
                .setStyle(ButtonStyle.Primary)
    ...
    _getCacheKey(query, userId) {
        return `amenities_results:${userId}:${this.normalizeQuery(query)}`;
    }
...
module.exports = AmenitiesResultsManager;